...
export const DIFFICULTY: Record<string, DifficultyInfo> = {
  // ============================================
  ...
  pair_88_A: { weight: 5, note: 'SPLIT 8s vs Ace!' },
...
const DEALER_CARD_VALUES: DealerCard[] = ['2', '3', '4', '5', '6', '7', '8', '9', '10', 'A'];

/**
 * Calculate default weight for hands not explicitly defined
 */
function getDefaultWeight(handType: string, handKey: string | number, _dealer: string): number {
  if (handType === 'hard') {
    const total = typeof handKey === 'number' ? handKey : parseInt(String(handKey));
    if (total >= 17 && _dealer !== 'A') return 1;  // Always stand
    if (total <= 8) return 1;  // Always hit
    if (total === 11 && _dealer !== 'A') return 1;  // Always double
  }
 ...
 */
export function getAllHands(): Hand[] {
  const hands: Hand[] = [];

  // Hard hands (5-21 vs 2-A)
  for (let total = 5; total <= 21; total++) {
    for (const dealer of DEALER_CARD_VALUES) {
 ...
 */
export function getWeightedHand(mode: TrainingMode = 'balanced'): Hand {
  const allHands = getAllHands();

  const modeMultipliers: Record<TrainingMode, Record<number, number>> = {
